...
const terminalTransferStatuses = new Set([
  "DELIVERED",
  "READ",
  "FAILED",
  "CANCELLED",
...
function isTerminalTransferStatus(status: string) {
...
function Workspace({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  useEffect(() => {
    const synchronize = async () => {
      ...
      if (storedDeviceID) {
        const challenge = await api.send<{
          id: string;
          sessionId: string;
          ephemeralPublicKey: string;
          nonce: string;
        }>(`/api/devices/${storedDeviceID}/session-challenge`, "POST");
        const proof = await proveDeviceSession(
          user.id,
        ...
        await api.send(
          `/api/devices/${storedDeviceID}/attach-session`,
          "POST",
          { challengeId: challenge.id, proof },
        );
      }
      await reload();
    };
  ...
  async function finishSend() {
    await reload();
    setSharedContent({ content: "", groupId: "" });
  }
...
function SendView({
  ...
  const trusted = devices.filter(
    (item) => item.trustStatus === "TRUSTED" && item.publicKey,
  );
  ...
  function chooseFiles(nextFiles: File[]) {
    setFiles(nextFiles);
    if (nextFiles.length) setContent("");
  }

  function dropFiles(event: React.DragEvent<HTMLFormElement>) {
    event.preventDefault();
    const dropped = Array.from(event.dataTransfer.files);
    if (dropped.length) chooseFiles(dropped);
  }
    ...
    try {
      ...
      if (files.length) {
        ...
        const transfer = await api.send<Transfer>("/api/transfers", "POST", {
          ...
          contentType: files.every((file) => file.type.startsWith("image/"))
            ? "IMAGE"
            : "FILE",
          routeMode: "AUTOMATIC",
  ...
  return (
    <section className="page send-page">
      <PageHeading
        eyebrow="QUICK DROP"
        title="今天要傳送什麼？"
        description="選擇信任設備，NexDrop 會自動判斷區網或節點路徑。"
      />
      <div className="send-grid">
        ...
        >
          ...
          <textarea
            ...
            placeholder="貼上文字、網址或想傳給另一台設備的內容…"
            maxLength={100000}
          />
          {!files.length && (
          ...
          <label className="file-input">
            ...
            <small>
              {files.length
                ? `${files.length} 個檔案 · ${formatBytes(files.reduce((total, file) => total + file.size, 0))}`
                : "可選擇或拖放圖片與一般檔案"}
            </small>
          </label>
          <div className="composer-meta">
            ...
            <span className="secure-pill">● 端對端加密</span>
          </div>
            ...
            {trusted.map((item) => (
              ...
              >
                ...
                <span>
                  ...
                  <small>{labelDeviceType(item.type)}</small>
                </span>
                <i>{selected.includes(item.id) ? "✓" : "+"}</i>
              </button>
            ))}
            {!trusted.length && (
              <Empty text="請先在「設備」輸入節點密鑰並登記此設備" />
            )}
          </div>
          <button
          ...
          >
            {busy ? (
              ...
              <>
                傳送給 {selected.length} 台設備 <span>↗</span>
              </>
            )}
          </button>
        </form>
...
type ActivityViewProps = Pick<
  ChatViewProps,
  "user" | "devices" | "transfers" | "reload"
>;
...
function ActivityView({
  ...
  const names = useMemo(
    () =>
      Object.fromEntries(devices.map((item) => [item.id, item.displayName])),
    [devices],
  );
  ...
  async function togglePause(transfer: Transfer) {
    ...
    if (paused) {
      pausedTransfers.delete(transfer.id);
    } else {
      pausedTransfers.add(transfer.id);
    }
    const activeTargets = transfer.targets.filter(
      (target) => !isTerminalTransferStatus(target.status),
    );
    await Promise.all(
  ...
  return (
    <section className="page">
      ...
      <div className="table-card card">
        ...
        {transfers.map((item) => {
          const firstFile = fileMetadata(decrypted[item.id], 0);
          const displayContent = item.files.length
            ? firstFile?.name ?? "加密檔案"
            : decrypted[item.id] ??
              (item.contentType === "TEXT" ? "加密文字" : item.contentType);
          const canManage =
            ...
            <article className="table-row" key={item.id}>
              <div>
                ...
                <p>
                  ...
                  <small>
                    {item.id.slice(0, 8)}
                    {item.files.length > 1
                      ? ` · ${item.files.length} 個檔案`
                      : ""}
                  </small>
                  {item.files.map((file, index) => {
                    const metadata = fileMetadata(decrypted[item.id], index);
                    return metadata ? (
                      <button
              ...
              <span>
                {item.targets
                  .map(
                    (target) =>
                      names[target.deviceId] ?? target.deviceId.slice(0, 8),
                  )
                  .join("、")}
              </span>
              <span className="route-label">
                {item.targets[0]?.selectedRoute ?? "—"}
              </span>
...
function DevicesView({ user, devices, reload, notify }: DevicesViewProps) {
  const [busy, setBusy] = useState(false);
  ...
  function importSettings(value: string) {
    ...
    try {
      ...
      const isJoinLink =
        parsed.protocol === "nexdrop:" && parsed.hostname === "join";
      setNodeKey(isJoinLink && key ? key : trimmed);
    } catch {
      setNodeKey(trimmed);
    }
  }
  ...
  async function revoke(id: string) {
    try {
      await api.send(`/api/devices/${id}/revoke`, "POST");
      await reload();
      notify("設備已撤銷");
    } catch (reason) {
      notify(messageFor(reason));
  ...
  async function copyImportValue() {
    try {
      await navigator.clipboard.writeText(importValue);
      notify("已複製節點導入資料");
    } catch {
      notify("無法存取剪貼簿，請手動複製");
    }
  }

  const importValue = `nexdrop://join?node=${encodeURIComponent(location.origin)}&key=${encodeURIComponent(nodeKey)}`;
  const registerLabel = busy
    ? "加入中…"
    : registered
      ? "此瀏覽器已加入"
      : "+ 加入此瀏覽器";
  return (
    <section className="page">
      <PageHeading
        eyebrow="NODE DEVICES"
        title="設備"
        description="設備只需要節點連結與節點密鑰即可加入；同一節點不再使用配對碼。"
        action={
          <button
            className="primary"
            onClick={register}
            disabled={busy || registered || !nodeKey.trim()}
          >
            {registerLabel}
          </button>
        }
      ...
      <div className="card settings-form">
        <div className="list-title">
          <div>
            <p className="eyebrow">NODE IMPORT</p>
            <h3>節點連結與密鑰</h3>
          </div>
          <button
            className="secondary"
            type="button"
            onClick={copyImportValue}
          >
            一鍵複製導入
          </button>
        </div>
        ...
        <label>
          ...
          <input
            type="password"
            value={nodeKey}
            onChange={(event) => importSettings(event.target.value)}
            placeholder="貼上節點密鑰或完整 nexdrop://join 資料"
          />
        </label>
        <label>
          完整導入資料
      ...
      <div className="cards-grid devices-grid">
        {devices.map((item) => (
          <article className="device-card card" key={item.id}>
            <div className="device-top">
              <DeviceGlyph type={item.type} />
              <Status value={item.online ? "ONLINE" : "OFFLINE"} />
            </div>
            <h3>{item.displayName}</h3>
            <p>
              {labelDeviceType(item.type)} ·{" "}
              {item.online
                ? "目前在線"
                : item.lastSeenAt
                  ? `最後上線 ${formatDate(item.lastSeenAt)}`
                  : "尚未連線"}
            </p>
            <div className="device-actions">
              <Status value={item.trustStatus} />
              {item.trustStatus !== "REVOKED" && (
                <button
                  className="text-danger"
        ...
        {!devices.length && <Empty text="尚未加入任何設備" />}
      </div>
    </section>
  );
}
...
function AnalyticsView() {
  ...
  const load = useCallback(async () => {
    ...
    const path = (endpoint: string) => {
      if (range.preset !== "custom" || !range.from || !range.to) {
        return statisticsPath(endpoint, Number(range.preset));
      }
    ...
    try {
      const [nextOverview, nextDaily, nextDevices] = await Promise.all([
        api.get<Overview>(path("/api/statistics/overview")),
        api.get<DailyTransfer[]>(path("/api/statistics/transfers")),
        api.get<DeviceStatistic[]>(path("/api/statistics/devices")),
      ]);
      setOverview(nextOverview);
      setDaily(nextDaily);
      setDeviceStats(nextDevices);
    } catch (reason) {
      setError(messageFor(reason));
    }
  }, [range]);
  ...
  return (
    <section className="page">
      <PageHeading
        eyebrow="TRANSFER ANALYTICS"
        title="傳輸統計"
        description="掌握流量、成功率與實際使用的傳輸路徑。"
        action={
          <div className="admin-tabs">
            <select
              value={range.preset}
              onChange={(event) =>
                setRange({ ...range, preset: event.target.value })
              }
            >
        ...
        <div className="card audit-list">
          ...
          {daily.map((item) => (
            <article key={item.date}>
              <span className="audit-mark">▥</span>
              <p>
                <strong>
                  {item.date} · {item.count} 次
                </strong>
                <small>
                  總計 {formatBytes(item.totalBytes)} · LAN{" "}
                  {formatBytes(item.lanBytes)} · 節點{" "}
                  {formatBytes(item.nodeBytes)}
                </small>
              </p>
              <Status value={item.failed ? "FAILED" : "DELIVERED"} />
            </article>
          ))}
        </div>
      )}
      {!!deviceStats.length && (
        <div className="card audit-list">
          <div className="list-title">
            <h3>每台設備狀態與傳輸用量</h3>
            <span>
              {deviceStats.filter((item) => item.online).length}／
              {deviceStats.length} 台在線
            </span>
          </div>
          {deviceStats.map((item) => {
            const availability = item.online
              ? "即時在線"
              : item.lastSeenAt
                ? `最後上線 ${formatDate(item.lastSeenAt)}`
                : "尚未連線";
            return (
              <article key={item.deviceId}>
                <DeviceGlyph type={item.deviceType} />
                <p>
                  <strong>{item.displayName}</strong>
                  <small>
                    {labelDeviceType(item.deviceType)} · 傳送 {item.sentCount} 筆／
                    {formatBytes(item.sentBytes)} · 接收 {item.receivedCount} 筆／
                    {formatBytes(item.receivedBytes)} · {availability}
                  </small>
                </p>
                <Status value={item.online ? "ONLINE" : "OFFLINE"} />
              </article>
      ...
      {error ? (
        ...
        <>
          <div className="metric-grid">
            <Metric
              label="傳輸任務"
              value={overview.transferCount.toLocaleString()}
              note="選定期間"
            />
            <Metric
              label="傳輸容量"
          ...
          <div className="admin-layout">
            <div className="card route-summary">
              ...
              {daily.map((item) => (
                <div className="route-bar" key={item.date}>
                  <span>{item.date.slice(5)}</span>
                  <i>
                    <b
                      style={{
                        width: `${Math.max(2, (item.totalBytes / peak) * 100)}%`,
                      }}
                    />
                  </i>
              ...
              {!daily.length && <Empty text="尚無每日資料" />}
            </div>
            <div className="card route-summary">
              <div>
                <p className="eyebrow">ROUTE MIX</p>
                <h3>傳輸路徑分布</h3>
              </div>
              {Object.entries(overview.routeCounts ?? {}).map(
                ([route, count]) => (
                  <div className="route-bar" key={route}>
                    ...
                    <i>
                      <b
                        style={{
                          width: `${Math.max(4, (count / Math.max(overview.transferCount, 1)) * 100)}%`,
                        }}
                      />
                    </i>
                    <strong>{count}</strong>
              ...
              {!Object.keys(overview.routeCounts ?? {}).length && (
                <Empty text="尚無足夠資料" />
              )}
            </div>
          </div>
...
function Brand() {
  return (
    <div className="brand">
      ...
      <strong>NexDrop</strong>
    </div>
  );
}

function DeviceGlyph({ type }: { type: string }) {
  const glyph = type.includes("ANDROID")
    ? "A"
    : type.includes("WEB")
      ? "W"
      : "▰";
  return <span className={`device-glyph ${type.toLowerCase()}`}>{glyph}</span>;
}

function Status({ value }: { value: string }) {
  const className = `status status-${value.toLowerCase().replaceAll("_", "-")}`;
...
function Metric({ label, value, note, danger }: MetricProps) {
  return (
...
function browserName() {
  const product = navigator.userAgent.includes("Edg/") ? "Edge" : "Chrome";
  return `${product} · ${navigator.platform || "Web"}`;
}
