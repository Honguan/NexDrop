...
export async function pairExtension(
  ...
  const currentDeviceAvailable = devices.some(
    (item) => item.id === id && item.trustStatus !== "REVOKED",
  );
  if (!id || !currentDeviceAvailable) {
...
export async function sendDirect(payload: SharePayload) {
  ...
  return request("/api/transfers", {
    ...
    body: JSON.stringify({
      targetType:
        targetIDs.length === 1 ? "SINGLE_DEVICE" : "MULTIPLE_DEVICES",
...
async function attachSession(id: string) {
  const challenge = await request<{
    ...
    nonce: string;
  }>(`/api/devices/${id}/session-challenge`, { method: "POST" });
  const keys = await ensureDeviceKey();
  const ephemeral = await crypto.subtle.importKey(
    "raw",
    fromBase64(challenge.ephemeralPublicKey),
    { name: "X25519" },
    false,
    [],
  );
  ...
  await request(`/api/devices/${id}/attach-session`, {
    method: "POST",
...
async function request<T = unknown>(
  ...
  try {
    ...
    if (shouldRefresh && (await refresh(origin, tokens.refreshToken))) {
...
async function refresh(origin: string, refreshToken: string) {
  try {
    const tokens = await raw<TokenPair>(origin, "/api/auth/refresh", {
      method: "POST",
...
async function ensureDeviceKey() {
  ...
  const value = stored[keyPairKey] as
    | { privateKey: JsonWebKey; publicKey: string }
    | undefined;
  if (value) {
    return {
...
async function encryptText(
  ...
  for (const recipient of recipients) {
    wrappedContentKeys[recipient.id] = await wrapKey(
      contentKey,
      recipient.publicKey,
    );
  }
