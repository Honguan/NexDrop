...
test("小視窗提供內容、網址選項與預設全選設備傳送，且不依賴桌面連線", async () => {
  ...
  assert.match(popup, /id="content-count"/);
  assert.match(popup, /id="include-url"/);
  assert.match(popupCode, /input\.type = "checkbox"/);
  assert.match(popupCode, /input\.checked = true/);
  assert.match(popupCode, /popupDraft/);
  assert.match(popupCode, /event\.ctrlKey \|\| event\.metaKey/);
  assert.match(popupCode, /chrome\.storage\.local\.remove\("popupDraft"\)/);
  assert.match(popupCode, /擴充功能尚未連接節點，請先開啟設定完成配對/);
  assert.match(popupCode, /未取得節點存取權限，請重新配對並允許網站權限/);
